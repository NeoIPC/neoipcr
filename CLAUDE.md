...
- **Always** mark a backwards-compatibility path with `NEOIPC-COMPAT(<scope>)` — one greppable token, a named scope, and a stated **removal condition**. <!-- SYNC: propagate to all repos --> This code exists because the deployed DHIS2 instance is older than the canonical metadata it is authored against, so a reader must accept both what is deployed *today* and what will be deployed *next*; that is a temporary state, and the only thing that reliably makes it temporary is being able to find every piece of it at once. Put the full reasoning on the **first** occurrence — what the deployment currently reports, what the canonical metadata says, and the condition under which the path becomes dead — and cross-reference it from the others (`# NEOIPC-COMPAT(<scope>): see <symbol>.`) rather than repeating it. Mark the **test** for the compatibility path too, so removing the path does not leave a test asserting behaviour that no longer exists. Two rules keep this from rotting: state the removal condition as a **fact about the world** that someone can check ("once every instance this package reads reports a code on all seven stages"), never as a date or a plan; and never widen a scope to cover a second concern — a new concern gets a new scope, because scopes are removed whole. Grep `NEOIPC-COMPAT` for the live inventory. **This is not a licence to keep a shim for our own API.** The package is pre-alpha and renames its own exported functions outright; compatibility is owed to **strings and data arriving from outside** — DHIS2 metadata, catalogue values, stored category codes — never to a caller of this package.
