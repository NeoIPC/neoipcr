...
- **Never** create a merge commit, and **always** squash-merge — a pull or merge request lands as exactly **one** commit, and a non-linear history is not permitted anywhere in this project. This is enforced in the forge settings rather than left to memory: merge commits are disabled, squash merging is enabled, the source branch is deleted on merge, and rebase merging is disabled too **unless this repository's own guardrails document a specific need for it** (one does, for catalogues whose translation platform retains and replays its own commits — where a squash destroys the patch identity that lets it recognise its work as already merged). GitLab projects express the same thing as `merge_method: ff` with `squash_option: always`. **Compose the squash message; never accept the pre-filled one.** The forge seeds it with every individual commit message from the branch, which is work-in-progress narration rather than a description of what landed. Replace it with one message that says what the change delivers and why, at the thematic level — naming a file or function only where it anchors a point, never as an inventory of everything touched.
