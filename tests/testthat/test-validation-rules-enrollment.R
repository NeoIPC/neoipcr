...
test_that("rule 43 judges whatever a surveillance-end bound left in the dataset", {
  # The period selects the enrolments before the pass; the rule reads the
  # records, not the bounds, so an active enrolment the dataset holds is
  # its to question.
  ds <- open_enrolment_ds(400)
  ds$metadata$dataset_options$surveillance_end_to <- as.Date("2025-12-31")
  expect_equal(nrow(neoipcr:::validation_rule_43(ds, NULL, open_enrolment_as_of)), 1L)
})
