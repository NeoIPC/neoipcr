...
| 32 | `validate` | [R/validation.R](R/validation.R) | `(x, rules = NULL, exceptions = NULL, as_of = NULL)` | tibble (`rule_id`, `patient_key`, `enrollment_key`, `event_key`, `context`) | external-stable | experimental | — | The 43 validation rules over a dataset; a finding is data (keys and a one-row `context` tibble), never prose. Aborts on a rule id it does not know. `as_of` is the date the two open-enrolment rules measure against, defaulting to the import's server date. |
